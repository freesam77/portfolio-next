...
const getBlocks = async (blockId: string, notionClient: Client): Promise<BlockObjectResponse[]> => {
...
const blocksToHTML = async (
	...
	const getNestedListHTML = async (
		...
		if (block.type === 'bulleted_list_item') {
			listBuffer.push(
				`<li>${extractRichText(block.bulleted_list_item.rich_text)}${
					nestedListHtml ? `\n${nestedListHtml}` : ''
				}</li>`,
			);
		} else if (block.type === 'numbered_list_item') {
			listBuffer.push(
				`<li>${extractRichText(block.numbered_list_item.rich_text)}${
					nestedListHtml ? `\n${nestedListHtml}` : ''
				}</li>`,
			);
		}
...
const notionPageToHTML = async (notionClient: Client, pageId: string): Promise<string | null> => {
